...
class FolderIndex:
    ...
    def getChildren(self, path):
        ...
        if self.INFO_FILENAME in children:
            yield self.INFO_FILENAME, False, False

        # add path to each file
        children = [ (f, os.path.join(path, f)) for f in children]
        children.sort(key=lambda x: os.path.getmtime(x[1]))
        ...
        for name, child_path in children:
            ...
            yield name, isdir, True

    def getTableRows(self, folder_name, path):
        ret = []

        for name, isdir, follow in self.getChildren(path):
            ...
            if isdir and follow:
                self.writeIndexFile(os.path.join(folder_name, name), os.path.join(path, name))
    ...
    def infoToHtmlWithTemplate(cls, info_key, info, template):
        if isinstance(info, dict):
            ...
            for key, value in iter(sorted(info.iteritems())):
    ...
    def getIndexHtml(self, folder_name, path):
        ...
        return self.main_index_html_template % dict(
            ...
            tablerows=self.getTableRows(folder_name, path),
    ...
    def writeIndexFile(self, folder_name, path):
        ...
        output_data = self.getIndexHtml(folder_name, path)
